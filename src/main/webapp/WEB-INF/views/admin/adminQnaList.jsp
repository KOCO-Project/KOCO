...
<script
	...
	function del() {
		...
		var form = document.qnaDeleteList;
		if(cnt == 0){
            alert("선택된 글이 없습니다.");
		}else{
		if (chk) {
...
<body>
	...
	<div class="container-fluid">
		<div class="row">
			...
			<div class="col-md-10">
				<h1><a href="adminQnaList" class="header"
							style="color: black; text-decoration: none; font-weight: bold;">Q&A게시판 관리</a></h1>
			</div>
			<div class="col-md-1"></div>
		</div>
	</div>
		...
		<div class="row">
			...
			<div class="col-md-10">
				<table class="table table-hover">
					...
					<form action="qnaDeleteList" method="post" name="qnaDeleteList">
						<c:forEach items="${qnaList }" var="qna">
								<tr class="table-light">
									<th><input type="checkbox" name="check" value="${qna.boardNo }"/></th>
									<th scope="row">${qna.boardNo }</th>
									<td>${qna.boardTitle }</td>
									<td>${qna.writer }</td>
									<td>${qna.boardRegdate }</td>
				...
				<table style="width: 100%">
					<tr>
						...
						<!-- 검색 영역 -->
						<td style="width: 40%;">
							<form class="navbar-form" action="adminQnaList" method="post">
					...
					<c:choose>
						...
						<c:otherwise>
							<li class="page-item"><a
								href="adminQnaList?infoNo=${infoNo }&page=${pageVO.prevPage }&searchCondition=${searchCondition}&searchKeyword=${searchKeyword}"
						...
						<c:choose>
							<c:when test="${idx == pageVO.currentPage }">
								<li class="page-item active"><a
									href="adminQnaList?infoNo=${infoNo }&page=${idx }&searchCondition=${searchCondition}&searchKeyword=${searchKeyword}"
